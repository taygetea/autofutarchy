"""
LLM-based trader with Exa search capability for the prediction market
"""
import requests
import json
...
class LLMTraderWithSearch:
    ...
    def search_for_context(self, question):
        ...
        try:
            print(f"[{self.name}] Searching for information about: {question[:50]}...")
            
            # Create a search query from the market question
            search_query = question.replace("Will ", "").replace("?", "") + " news predictions 2025"
            ...
            print(f"[{self.name}] Found {len(context)} relevant sources")
            return context
    ...
    def analyze_market(self, market):
        """Use LLM to analyze a prediction market with optional search context"""
        
        # Search for relevant information
        search_context = self.search_for_context(market['question']) if self.use_search else None
        
        # Build search context string
        search_info = ""
        if search_context:
            search_info = "\n\nRECENT INFORMATION FROM WEB SEARCH:\n"
            for i, ctx in enumerate(search_context, 1):
                search_info += f"\n{i}. {ctx['title']}\n   Source: {ctx['url']}\n   {ctx['snippet']}...\n"
...
- Pool liquidity: You can buy at most ~{int(market['yes_pool'] * 0.5)} YES shares or ~{int(market['no_pool'] * 0.5)} NO shares
  (buying more would exhaust the pool){search_info}
...
def run_llm_traders_with_search(market_id, num_traders=3, rounds=1, enable_search=True):
    """Run multiple LLM traders on a market with optional search"""
    
    # Different models and strategies (using modern models from orchestrator)
    trader_configs = [
        ("openrouter/google/gemini-2.5-flash", "aggressive", True),
        ("openrouter/openai/o4-mini", "balanced", True),
        ("openrouter/anthropic/claude-sonnet-4", "conservative", False),  # No search for one
        ("openrouter/deepseek/deepseek-r1-0528", "balanced", True),
        ("openrouter/moonshotai/kimi-k2", "aggressive", True),
        ("openrouter/google/gemini-2.5-pro", "analytical", True),
        ("openrouter/openai/gpt-4.1", "conservative", True),
    ]
    
    # Create traders
    traders = []
    for i in range(min(num_traders, len(trader_configs))):
        model, strategy, use_search = trader_configs[i]
        trader = LLMTraderWithSearch(
            model=model, 
            strategy=strategy,
            use_search=use_search and enable_search
        )
        if trader.register():
    ...
    if enable_search:
        print("Web search enabled for market analysis")
    ...
    for round_num in range(rounds):
        ...
        for trader in traders:
            ...
            time.sleep(2)  # Rate limiting for Exa API
    ...
    for trader in traders:
        ...
        if resp.status_code == 200:
            user = resp.json()
            print(f"\n{trader.name} ({'with search' if trader.use_search else 'no search'}):")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python llm_trader_with_search.py <market_id> [num_traders] [rounds] [--no-search]")
        print("Example: python llm_trader_with_search.py market_3 3 2")
        print("         python llm_trader_with_search.py market_3 3 2 --no-search")
        sys.exit(1)
    
    market_id = sys.argv[1]
    num_traders = int(sys.argv[2]) if len(sys.argv) > 2 else 3
    rounds = int(sys.argv[3]) if len(sys.argv) > 3 else 1
    enable_search = "--no-search" not in sys.argv
    
    run_llm_traders_with_search(market_id, num_traders, rounds, enable_search)